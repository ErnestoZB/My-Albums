...
class ChooseImagesViewModel(val context: Context) : ViewModel()  {
    ...
    val launchSortImagesActivity = MutableLiveData<Event<Boolean>>()

    fun onReadPermissionDenied() {
        errorMessage.postValue(Event(noReadPermissionError))
    }
    ...
    fun onSortImagesClicked() {
        ...
        {
            errorMessage.postValue(Event(noImagesSelectedError))
            return
        }

        filterSelectedImages()

        launchSortImagesActivity.postValue(Event(true))
    }
    ...
    fun getDeviceImages() = GlobalScope.async {
        ...
        {
            errorMessage.postValue(Event(noAvailableImagesError))
        }
